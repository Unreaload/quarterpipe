  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch('/api/contact', {
        ...
        body: JSON.stringify({ formType: 'Raumanfrage', ...payload }),
      });
    ...
    <form className="px-8 md:px-16 py-10 flex flex-col gap-0" onSubmit={handleSubmit}>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 pt-12 pb-16">
        <div className="flex items-start gap-4 max-w-lg">
          <input type="checkbox" required id="datenschutz-mieten" name="privacy" className="mt-1 w-5 h-5 shrink-0 cursor-pointer accent-black" />
          <label htmlFor="datenschutz-mieten" className="font-mono text-[10px] leading-relaxed opacity-50 cursor-pointer tracking-wide">
